...
import { saveRefreshToken, signAccessToken } from "../../auth/auth.service.ts";
import { sendEmail, sendResetPasswordEmail } from "../../config/resend/resend.ts";
...
export const profileByUser = async (args: { id: string }, context: Context) => {
  try {
    if (!context.user || !context.user.userId) throw new Error('No autorizado');
    const { id } = args;
...
export const searchUser = async (args: { query: string }, context: Context) => {
  try {
    if (!context.user || !context.user.userId) throw new Error('No autorizado');
    const { query } = args;
...
export const getUsersNotFollowing = async (context: Context) => {
  try {
    if (!context.user || !context.user.userId) throw new Error('No autorizado');
    const res = await userModel.getUsersNotFollowing(context);
    return res.rows;
  } catch (error) {
    console.error('Error al obtener usuarios no seguidos', error);
    throw error;
  }
}

export const register = async (args: { name: string, email: string, birthday: string, password: string }) => {
  try {
...
export const login = async (args: { email: string, password: string }, context: Context) => {
  try {
...
export const resetPassword = async (args: { password: string, token: string }) => {
  try {
...
export const verifyEmail = async (args: { token: string }, context: Context) => {
  try {
    const { token } = args;
