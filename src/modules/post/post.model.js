...
export const postsByUser = async (afterDate, limit) => {
  let query;
  let values;
  if (afterDate) {
...
export const countPostsByUser = async (userId) => {
  return pool.query(`SELECT COUNT(*) AS count FROM posts WHERE user_id = $1`,
    [userId]);
}

export const followingResultId = async (context) => {
  return pool.query(`SELECT friend_id FROM friends WHERE user_id = $1`,
    [context.user.userId]);
}

export const feedPosts = async (userIds, afterDate, limit) => {
  let query;
  let values;
  if (afterDate) {
    query = `SELECT * FROM posts 
                 ...
                 ORDER BY created_At DESC LIMIT $3`;
    values = [userIds, afterDate, limit + 1];
  } else {
    query = `SELECT * FROM posts 
                 WHERE user_id = ANY($1)
                 ORDER BY created_at DESC LIMIT $2`;
    values = [userIds, limit + 1];
  }
  return pool.query(query, values);
}
...
export const insertPost = async (args, context) => {
...
export const insertMedia = async (post, media) => {
  const { id } = post;
...
export const insertarNotificationRepost = (userId, postId, type, reference, context) => {
...
export const countReactionss = async (id) => {
  return pool.query(`SELECT * FROM comments WHERE post_id = $1`, [id])
}
