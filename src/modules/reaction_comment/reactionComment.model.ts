...
export const createCommentReaction = async (commentId: string, context: Context) => {
  return pool.query(
...
export const deleteCommentReaction = async (commentId: string, context: Context) => {
  return pool.query(`DELETE FROM reactions_comments WHERE comment_id = $1 AND user_id = $2
        RETURNING id, comment_id, user_id`,
        [commentId, context.user?.userId]);
}

export const insertNotification = async (userId: string, commentId: string, context: Context) => {
