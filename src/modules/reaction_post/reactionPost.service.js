import * as reactionPostModel from './reactionPost.model.js';
import { pubsub } from '../../config/pubsub.js';
import { getPostById } from '../post/post.model.js';

export const createPostReaction = async (args, context) => {
  try {
...
export const deletePostReaction = async (args, context) => {
  try {
    if (!context.user || !context.user.userId) throw new Error('No autorizado');
    const { postId } = args;
